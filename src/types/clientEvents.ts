...
export type PlaybackStall = {
  ...
  // chat events
  interface ChatSendEventType {
    chatRoomId: number;
    totalChatMsg: number;
  }
  
  interface BannerClickEventType {
    bannerId: number;
    bannerType: number;
  }
  
  type BannerViewEventType = BannerClickEventType
  
  interface PartyUpClickEventType {
    channelId: number;
    gameId: number;
    gameUrl: string;
  }
  ...
  interface BackgroundEventType {
    activeUsageDuration: number;
  }
  
  // report active session event
  interface ActiveSessionEventType {
    duration: number;
  }
  
  // vod events
  interface VodDownloadEventType {
    vodId: string;
    vodSize: number;
    vodLength: number;
    gameBuildId: number;
  ...
  interface VodShareEventType {
    vodId: string;
    sharingPlatform: SharingPlatform;
    gameBuildId: number;
  }
  
  interface VodLikeEventType {
    vodId: string;
    gameBuildId: number;
  }
  
  interface VodPublishEventType {
    vodId: string;
    gameBuildId: number;
    vodLength: number;
    status: EventStatus;
  ...
  interface ClipDownloadEventType {
    videoId: string;
    status: EventStatus;
    videoSize: number;
    videoLength: number;
  ...
  interface ClipLikeEventType {
    videoId: string;
    videoDisplayType: number;
    gameBuildId: number;
  }
  interface ClipDisplayEventType {
    source: EventSource;
    feedPosition: number;
    stats: PlaybackStats;
  }
  ...
  // highlight events
  interface HighlightViewEventType {
    highlightId: string;
    viewDuration: number;
    gameBuildId: number;
  ...
  interface HighlightDownloadEventType {
    highlightId: string;
    status: EventStatus;
    size: number;
    length: number;
  ...
  interface HighlightShareEventType {
    highlightId: string;
    sharingPlatform: SharingPlatform;
    gameBuildId: number;
  }
  ...
  // stream events
  interface StreamViewEventType {
    channelId: number;
    gameBuildId: number;
    viewDuration: number;
  }
  
  interface HLSDownloadEventType {
    channelId: number;
    ts: TS[];
    m3u8: M3U8[];
  }
  
  interface StreamQualityStallEventType {
  ...
  interface StreamQualityStartupEventType {
    watchChannelId: number;
    domain: string;
    startupDuration: number;
    selectedResolution: string;
  ...
  interface StreamQualityGiveupEventType {
    watchChannelId: number;
    stalls: StreamStall[];
  ...
  interface StreamShareEventType {
    channelId: number;
  ...
  interface StreamErrorEventType {
    watchChannelId: number;
    domain: string;
    selectedResolution: string;
    playResolution: number;
