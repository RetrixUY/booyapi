...
export default {
  alertConfigs: {
    get(
      ...
      return apiv3('get', `channels/${channelId}/alert-configs`,{},sessionKey);
    },
    patch(channelId: number, config: Partial<AlertConfig>,sessionKey: string): Promise<void> {
      return apiv3('patch', `channels/${channelId}/alert-configs`, {
        ...config
      },sessionKey);
    },
    getTab(
      ...
      return apiv3('get', `channels/${channelId}/alert-configs/${tab}`,{},sessionKey);
    },
    postTest(channelId: number, tab: AlertBannerTab,sessionKey: string): Promise<{}> {
      return apiv3('post', `channels/${channelId}/alert-config-test/${tab}`,{},sessionKey);
    }
  },
  ffLuckyDraws: {
    get(
      ...
      return apiv3('get', `channels/${channelId}/ff-lucky-draws`, { cursor },sessionKey);
    },
    post(
      channelId: number,
      luckyDrawInfo: {
    ...
    getLogs(
      ...
      return apiv3('get', `channels/${channelId}/ff-lucky-draws/logs`, {
        cursor,
        count
      },sessionKey);
    },
    getRoundHistory(
      ...
      return apiv3('get', `channels/${channelId}/ff-lucky-draw-rounds`, {
        cursor,
        count
      },sessionKey);
    },
    getRound(
      ...
      return apiv3('get', `channels/${channelId}/ff-lucky-draw-ongoing-round`,{},sessionKey);
    },
  ...
  firebaseTokens: {
    post(channelId: number,sessionKey: string): Promise<{ token: string }> {
      return apiv3('post', `channels/${channelId}/firebase-tokens`,{},sessionKey);
    }
  },
  getItem(
    channelId: number | string,sessionKey: string
  ): Promise<{ user: User; channel: Channel }> {
    return apiv3('get', `channels/${channelId}`,{},sessionKey);
  },
  gifts: {
    getLogs(
      channelId: number,
      ...
      return apiv3('get', `channels/${channelId}/gifts/logs`, {
        txnType: logType,
        orderType,
        startTime,
        endTime,
    ...
    getTopGifters(
      ...
      return apiv3('get', `channels/${channelId}/gifts/top-gifters`, {
        type
      },sessionKey);
    }
  },
  patch(
    ...
    return apiv3('patch', `channels/${channelId}`, channel,sessionKey);
  },
  getStats(
    channelId: number,
    startTime: number,
    ...
    return apiv3('get', `channels/${channelId}/stats`, {
      startTime,
      endTime
    },sessionKey);
  },
  streams: {
    get(channelId: number,sessionKey: string): Promise<Stream> {
      return apiv3('get', `channels/${channelId}/streams`,{},sessionKey);
    }
  },
  streamKeys: {
    post(channelId: number,sessionKey: string): Promise<{ streamKey: string; url: string }> {
      return apiv3('post', `channels/${channelId}/stream-keys`,{},sessionKey);
    }
  },
  streamSettings: {
    get(channelId: number,sessionKey: string): Promise<StreamSettings> {
      return apiv3('get', `channels/${channelId}/stream-settings`,{},sessionKey).then(
        (res: StreamSettings) => {
          if (!res.resolution) {
            res.resolution = '720p';
          }
  ...
  postVideos(
    ...
    return apiv3('post', `channels/${channelId}/videos`, params,sessionKey);
  },
  postClips(
    channelId: number,
    params: ClipUploadParam,sessionKey: string
  ): Promise<{ clipId: string }> {
    return apiv3('post', `channels/${channelId}/clips`, params,sessionKey);
  },
  hostees: {
    getItem(channelId: number,sessionKey: string): Promise<any> {
      return apiv3('get', `channels/${channelId}/hostees`,{},sessionKey);
    },
    post(channelId: number, hosteeId: number,sessionKey: string): Promise<void> {
      return apiv3('post', `channels/${channelId}/hostees`, {
        channelId: hosteeId
      },sessionKey);
    },
    delete(channelId: number,sessionKey: string): Promise<void> {
      return apiv3('delete', `channels/${channelId}/hostees`,{},sessionKey);
    }
  },
  savedHostees: {
    get(
      ...
      return apiv3('get', `channels/${channelId}/saved-hostees`, params,sessionKey);
    },
    post(channelId: number, hosteeId: number,sessionKey: string): Promise<void> {
      return apiv3('post', `channels/${channelId}/saved-hostees`, {
        channelId: hosteeId
      },sessionKey);
    },
    delete(channelId: number, hosteeId: number,sessionKey: string): Promise<void> {
      return apiv3('delete', `channels/${channelId}/saved-hostees/${hosteeId}`,{},sessionKey);
    }
  },
  socialLinks: {
    post(
      ...
      return apiv3('post', `channels/${channelId}/social-links`, {
        platform,
        link
      },sessionKey);
    },
    delete(channelId: number, platform: SocialPlatform,sessionKey: string): Promise<void> {
      return apiv3('delete', `channels/${channelId}/social-links`, {
        platform
      },sessionKey);
    }
  },
  streamerPrograms: {
    get(
      ...
      return apiv3('get', `channels/${channelId}/streamer-programs`, {
        recordOffset
      },sessionKey);
    },
    post(channelId: number,sessionKey: string) {
      return apiv3('post', `channels/${channelId}/streamer-programs`,{},sessionKey);
    },
    payouts: {
      get(
        channelId: number,sessionKey: string
        ...
        return apiv3('get', `channels/${channelId}/streamer-programs/payouts`,{},sessionKey);
      },
      post(
        channelId: number,
        recordId: number,
  ...
  pinnedPlaybacks: {
    get(
      ...
      return apiv3('get', `channels/${channelId}/pinned-playbacks`, {
        type,
        gameId,
        contentLanguage
      },sessionKey);
