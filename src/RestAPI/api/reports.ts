...
export default {
  ...
  post(
    ...
    type: ReportType,
    refId: string | number,
    reason: number,sessionKey: string,
    reportedUid?: string,
    reportedSource?: StreamingPlatform
  ): Promise<void> {
    return apiv3('post', 'reports', {
      reason,
  ...
  bugs: {
    post(
      ...
      eventTime: number,
      logFileList: string[],
      screenshotsList: string[],sessionKey: string
    ): Promise<void> {
      return apiv3('post', 'reports/bugs', {
        uid: userUID,
        description,
        email,
        logFileList,
  ...
  feedbacks: {
    post(
      ...
      return apiv3('post', 'reports/feedbacks', {
        uid: userUID,
        description,
        email,
        screenshotsList,
  ...
  clientEvents: {
    post(
      ...
      return apiv3('post', 'reports/client-events', {
        clientVersion: 'web',
        deviceId,
        events,
        language,
  ...
  clientHeartbeat: {
    post(sessionKey: string): Promise<void> {
      return apiv3('post', 'reports/client-heartbeats',{},sessionKey);
    }
  },
  embedConnects: {
    post(channelId: number, deviceId: string,sessionKey: string): Promise<void> {
      return apiv3('post', 'reports/embed-connects', {
        channelId,
        deviceId,
        uid: deviceId
      },sessionKey);
  ...
  embedHeartbeat: {
    post(channelId: number, deviceId: string,sessionKey: string): Promise<void> {
      return apiv3('post', 'reports/embed-heartbeats', {
        channelId,
        deviceId,
        uid: deviceId
      },sessionKey);
  ...
  comments: {
    post(params: {
      ...
      return apiv3('post', 'reports/comments', params,sessionKey);
    }
  }
};
